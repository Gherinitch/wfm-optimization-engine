...
const getDStyles = (density: Density) =>
  ({
    compact: {
      ...
      actionBadge: "text-[10px] px-1 py-0.5",
      rowHeight: 45,
    },
    normal: {
      thPad: "py-3 px-4",
      ...
      actionBadge: "text-[11px] px-1.5 py-0.5",
      rowHeight: 60,
    },
    large: {
      thPad: "py-4 px-5",
...
export default function WeeklyBalancing() {
  ...
  const [sortBy, setSortBy] = useState<
    "name" | "utilization_desc" | "utilization_asc"
  >("name");
  ...
  const loadData = useCallback(async () => {
    ...
    const dynamicStart = bounds[0].minDate as string;
    const dynamicEnd = bounds[0].maxDate as string;
  ...
  const handleAutoOptimize = async () => {
    if (metrics?.days?.length > 0) {
      ...
      if (moves && moves.length > 0) {
        setPendingInterdayOptimization({
          startDate: start,
          endDate: end,
          moves,
        });
  ...
  const handleCellClick = useCallback(
    async (agentId: string, date: string, hasShift: boolean) => {
      if (isMovingRef.current) return;
      const currentMove = moveStateRef.current;

      if (hasShift) {
        if (
          currentMove?.agentId === agentId &&
          currentMove?.sourceDate === date
        ) {
          setMoveState(null);
        } else {
  ...
  if (isEmpty)
    return (
      <div className="h-screen w-screen bg-background flex flex-col items-center justify-center font-mono text-status-danger gap-4">
        ...
        <Link
          href="/"
          className="px-6 py-3 bg-surface border border-surfaceBorder rounded-lg text-white hover:bg-surfaceBorder transition-colors"
        >
  ...
  return (
    <div className="h-screen flex flex-col bg-background text-gray-200 overflow-hidden font-sans">
      ...
      <div
        ref={parentRef}
        className="flex-1 overflow-auto custom-scrollbar relative will-change-scroll"
      >
        <div
          style={{ minWidth: "fit-content" }}
          className="group/table"
          data-moving={!!moveState}
        >
          {/* CSS Grid Header */}
          <div
            style={{ ...gridColsStyle, zIndex: Z_INDEX.TABLE_STICKY_HEADER }}
            className={`sticky top-0 border-b-2 border-surfaceBorder bg-surface shadow-sm w-full`}
          >
            <div
              className={`${dStyles.thPad} border-r border-surfaceBorder sticky left-0 bg-surface transform-gpu flex items-center`}
            ...
            {metrics.days.map((day: any) => {
              ...
              return (
                <div
                  key={day.date}
                  className="border-r border-surfaceBorder bg-surface bg-clip-padding relative flex flex-col items-center justify-center"
                >
                  <div
                    className={`flex flex-col items-center justify-center ${dStyles.thPad} w-full`}
                  >
                    <span
                      className={`font-medium text-gray-100 ${dStyles.headerDate}`}
                    >
                      {new Date(day.date).toLocaleDateString("en-US", {
                        weekday: "short",
                      })}{" "}
                      <span className="text-gray-500 text-[0.85em] font-normal tabular-nums">
                        {day.date.substring(5)}
                      </span>
                    </span>
                    <div className="flex items-center gap-1.5 mt-1 tabular-nums">
                      <span
                        className={`font-medium ${colorClass} ${dStyles.headerCov}`}
                      >
                        {day.coveragePct}%
                      </span>
                      {!day.isWithinTolerance && (
                        <span
                          className={`rounded font-medium ${dStyles.actionBadge} ${isOver ? "bg-status-danger/10 text-status-danger" : "bg-status-info/10 text-status-info"}`}
                        >
                          {day.hoursToMove > 0 ? "+" : ""}
                          {day.hoursToMove}h
                        </span>
              ...
              {rowVirtualizer.getVirtualItems().map((virtualRow) => {
                ...
                return (
                  <AgentRow
                    ...
                    dStyles={dStyles}
                    activeMove={
                      moveState?.agentId === agent.id ? moveState : null
                    }
