...
export const SegmentBlock = ({ segmentId }: { segmentId: string }) => {
  ...
  // Hides invisible structural containers to prevent visual clutter
  if (segment.isGeneral) return null;
  ...
  switch (segment.category) {
    case "Work":
      ...
      break;
    case "Break":
      bgClass = "bg-status-warning hover:bg-yellow-500 shadow-sm";
      break;
    case "Lunch":
  ...
  return (
    ...
    >
      <motion.div
        className={`absolute h-8 top-2 left-0 rounded-md cursor-grab active:cursor-grabbing flex items-center justify-center overflow-hidden transition-colors ${bgClass}`}
        // FIXED: Bind zIndex directly to eWFM Rank.
        // If dragging, bump it to 999 so it floats over everything else!
        style={{
          width,
          x: originalX + dragOffset,
          zIndex: isDragging ? 999 : segment.rank * 10,
        }}
        drag="x"
        dragMomentum={false}
        dragElastic={0}
