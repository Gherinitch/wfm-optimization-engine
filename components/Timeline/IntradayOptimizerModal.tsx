...
export const IntradayOptimizerModal = () => {
  const pendingIntradayOptimization = useScheduleStore((state) => state.pendingIntradayOptimization);
  const setPendingIntradayOptimization = useScheduleStore((state) => state.setPendingIntradayOptimization);
  const confirmIntradayOptimization = useScheduleStore((state) => state.confirmIntradayOptimization);
  ...
  const handleConfirm = () => {
    const approvedMoves = pendingIntradayOptimization.moves.filter(m => !rejectedMoves.has(m.segmentId));
    ...
    <div className="fixed inset-0 z-[999999] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-surface border border-surfaceBorder rounded-lg shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col">
        <div className="p-4 border-b border-surfaceBorder bg-surface/50">
          ...
          <p className="text-xs text-gray-400 mt-1">Review the system proposed break movements designed to minimize coverage variance.</p>
        </div>
          ...
          <table className="w-full text-left text-sm border-collapse">
            <thead>
              <tr className="border-b border-surfaceBorder">
                ...
                <th className="pb-2 font-medium text-gray-400 text-xs text-left">Agent</th>
                <th className="pb-2 font-medium text-gray-400 text-xs">Break</th>
                <th className="pb-2 font-medium text-gray-400 text-xs">Old Time</th>
                <th className="pb-2 font-medium text-gray-400 text-xs">New Time</th>
              </tr>
            </thead>
              ...
              {pendingIntradayOptimization.moves.map((move, i) => {
                ...
                return (
                  <tr key={i} className={`group ${isRejected ? 'opacity-40' : 'hover:bg-surface/50'} transition-opacity`}>
                    ...
                    <td className="py-3 text-xs text-gray-300 font-mono">{seg.name}</td>
                    <td className="py-3 text-red-300 line-through text-xs font-mono">{formatTime(seg.startMin)} - {formatTime(seg.endMin)}</td>
                    <td className="py-3 text-green-300 font-bold text-xs font-mono">{formatTime(move.newStart)} - {formatTime(move.newEnd)}</td>
                  </tr>
                );
              })}
            </tbody>
        ...
        <div className="p-4 border-t border-surfaceBorder bg-surface/80 flex items-center justify-between">
          <div className="text-sm font-medium">
            <span className="text-gray-400">Total Moves:</span> <span className="text-white">{pendingIntradayOptimization.moves.length - rejectedMoves.size}</span>
          </div>
