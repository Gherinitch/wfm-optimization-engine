...
export const InterdayOptimizerModal = ({ onComplete }: { onComplete: () => Promise<void> }) => {
  const pendingInterdayOptimization = useScheduleStore((state) => state.pendingInterdayOptimization);
  ...
  const handleConfirm = async () => {
    setIsApplying(true);
    const approvedMoves = pendingInterdayOptimization.moves.filter((_, i) => !rejectedMoves.has(i));
      ...
      <div className="bg-surface border border-surfaceBorder rounded-lg shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col">
        <div className="p-4 border-b border-surfaceBorder bg-surface/50">
          <h2 className="text-lg font-heading font-bold text-white flex items-center gap-2">
            ...
            Auto Days Off ({pendingInterdayOptimization.startDate} - {pendingInterdayOptimization.endDate})
          </h2>
          <p className="text-xs text-gray-400 mt-1">Review system proposed day-off reassignments based on coverage variance.</p>
        </div>
        ...
        <div className="flex-1 overflow-y-auto max-h-[60vh] p-4 text-sm">
          <table className="w-full text-left text-sm border-collapse">
            <thead>
              <tr className="border-b border-surfaceBorder">
                <th className="pb-2 font-medium text-gray-400 w-8 text-center text-xs">OK</th>
                <th className="pb-2 font-medium text-gray-400 text-xs pl-2">Agent ID</th>
                <th className="pb-2 font-medium text-gray-400 text-xs">Moving From (Overstaffed)</th>
                <th className="pb-2 font-medium text-gray-400 text-xs">Moving To (Understaffed)</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-surfaceBorder">
              {pendingInterdayOptimization.moves.map((move, i) => {
                ...
                return (
                  <tr key={i} className={`group ${isRejected ? 'opacity-40' : 'hover:bg-surface/50'} transition-opacity`}>
                    ...
                    <td className="py-3 font-medium font-mono text-xs pl-2">{move.agentId.substring(0,6)}...</td>
                    <td className="py-3 text-red-300 line-through text-xs">{move.oldDate}</td>
                    <td className="py-3 text-green-300 font-bold text-xs">{move.newDate}</td>
                  </tr>
                );
              })}
            </tbody>
          ...
          {pendingInterdayOptimization.moves.length === 0 && (
             <div className="text-center py-8 text-gray-400 font-mono text-sm">No moves available or schedule is fully constrained.</div>
          )}
        </div>

        <div className="p-4 border-t border-surfaceBorder bg-surface/80 flex items-center justify-between">
          <div className="text-sm font-medium">
            <span className="text-gray-400">Total Swaps:</span> <span className="text-white">{pendingInterdayOptimization.moves.length - rejectedMoves.size}</span>
          </div>
          <div className="flex gap-2">
            ...
            <button 
              ...
              disabled={isApplying || (pendingInterdayOptimization.moves.length - rejectedMoves.size === 0)}
