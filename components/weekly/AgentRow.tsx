...
export const AgentRow = memo(({ agent, metrics, dStyles, activeMove, onCellClick, style, gridColsStyle }: any) => {
  return (
    <div 
    ...
    >
      <div className={`${dStyles.cellPad} border-r border-surfaceBorder/50 sticky left-0 z-[40] bg-background group-hover/row:bg-surface transform-gpu transition-opacity duration-150 flex flex-col justify-center ${!activeMove ? 'group-data-[moving=true]/table:opacity-30' : '!opacity-100'}`}>
        <div className={`font-medium text-gray-300 truncate w-full ${dStyles.agentText}`}>{agent.name}</div>
        <div className="text-[10px] text-gray-500 font-mono tracking-tighter tabular-nums mt-0.5">{agent.totalWorkHours}h utilized</div>
      </div>
